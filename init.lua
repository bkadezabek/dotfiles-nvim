...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    dependencies = {
      ...
      -- WARN: Error when reloading -> Disables plugin for no reason in lazy
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      -- Suppress Diagnostic tokens from LSP -- Custom!
      vim.api.nvim_create_autocmd('LspAttach', {
                ...
                pylsp = {
                  plugins = {
                    pycodestyle = {
                      ...
                      maxLineLength = 100, -- Optional: Set a custom max line length
                    },
                  },
                },
              }
  ...
  { -- Autocompletion
    ...
    config = function()
      ...
      -- NOTE: Here is the list of filetypes that have problematic <CR> insert where it deletes the suggested autocomplete on <CR>
      local manual_insert_filetypes = { java = true }
      ...
      cmp.setup {
        ...
        snippet = {
          expand = function()
            -- Disabled so that functions do not insert arguments as names - ADD args to expand = function(args)!
        ...
        mapping = cmp.mapping.preset.insert {
          ['<Tab>'] = cmp.mapping(function(fallback)
            if cmp.visible() then
              cmp.select_next_item() -- Tab selects the next item
            else
              fallback()
            end
          end, { 'i', 's' }),
          ['<S-Tab>'] = cmp.mapping(function(fallback)
            if cmp.visible() then
              cmp.select_prev_item() -- Shift+Tab selects the previous item
            else
              fallback()
            end
          end, { 'i', 's' }),
          ['<Enter>'] = function(fallback)
            ...
            if cmp.visible() then
              ...
              if entry then
                if manual_insert_filetypes[ft] then
                  -- Java: Manually insert completion text
                  local word = entry.completion_item.insertText or entry.completion_item.label
                  cmp.confirm { select = true } -- Confirm selection
                  vim.defer_fn(function()
                    vim.api.nvim_put({ word }, 'c', true, true) -- Manually insert the text
                  end, 10) -- Delay to ensure it gets inserted
                else
                  -- Normal confirmation for all other languages
  ...
  --
  -- Debug Adapter, specifically for Godot
  require 'kickstart.plugins.debug',
